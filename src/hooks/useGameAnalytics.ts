...
export function useGameAnalytics(options: UseGameAnalyticsOptions) {
  ...
  const startSession = useCallback(() => {
    ...
    const sessionId = analyticsService.startGameSession(avatarId, gameType, settings as unknown as Record<string, unknown>, orgId);
   ...
   */
  const trackQuestionAttempt = useCallback((isCorrect: boolean, questionData?: Record<string, unknown>) => {
   ...
   */
  const trackEvent = useCallback((eventType: 'game_start' | 'game_complete' | 'question_start' | 'hint_used' | 'game_pause' | 'game_resume' | 'difficulty_change' | 'game_abandon', eventData?: Record<string, unknown>) => {
    if (!state.sessionId || !state.isTracking) {
      console.warn('No active session for event tracking');
      return;
    }

    analyticsService.trackEvent(state.sessionId, avatarId, eventType, eventData || {});
  }, [state.sessionId, state.isTracking, avatarId]);
  ...
  const completeSession = useCallback((finalScore?: number) => {
    ...
    analyticsService.completeGameSession(
      state.sessionId,
      calculatedScore,
      questionsAttempted.current,
      questionsCorrect.current
    ...
    // Note: These will be called directly to avoid circular dependencies
    setTimeout(() => {
      const recommendations = analyticsService.getLearningPathRecommendations(avatarId, 5);
      const metrics = analyticsService.getPerformanceMetrics(avatarId);
      setState(prev => ({
        ...prev,
        recommendations,
  ...
  const abandonSession = useCallback(() => {
    ...
    analyticsService.trackEvent(state.sessionId, avatarId, 'game_abandon', {
